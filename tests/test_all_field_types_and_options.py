...
def test_record_date_missing(fixture_all_field_types_and_options):
    res = fixture_all_field_types_and_options.execute(
        "SELECT * FROM record_error_datas WHERE record_id='date_missing' ORDER BY data_path ASC, message ASC"
    )
    errors = res.fetchall()
    assert len(errors) == 1
    assert errors[0]["data_path"] == ""
    assert errors[0]["message"] == "'date' is a required property"
